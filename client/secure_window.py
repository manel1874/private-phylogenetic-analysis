...
from interface.secure_window import Secure

### Common class
from common.s_compute import Secure_Compute_Window

class Secure_window(qtw.QWidget):
    ...
    def on_stateChanged(self, state):
        if state == QtNetwork.QAbstractSocket.ConnectedState:
            if self.ui.lineEdit_4.text() != "":
                msg = "Secure Multiparty " + self.ip + ' and ' + self.ui.lineEdit_4.text() + " Number of Sequences: " + str(len(self.files))
            else:
                msg = "Secure Multiparty " + self.ip + " Number of Sequences: " + str(len(self.files))
    ...
    def on_stateChanged2(self, state):
        if state == QtNetwork.QAbstractSocket.ConnectedState:
            if self.ui.lineEdit_3.text() != "":
                msg = "Secure Multiparty " + self.ip + ' and ' + self.ui.lineEdit_3.text()  + " Number of Sequences: " + str(len(self.files))
            else:
                msg = "Secure Multiparty " + self.ip + " Number of Sequences: " + str(len(self.files))
    ...
    def compute(self):
        ...
        view = Secure_Compute_Window(self.title,self.type, self.widget, self.numOfParties, self.id, numOfInputs)
