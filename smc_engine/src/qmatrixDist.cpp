...
matrixDist::matrixDist(shared_ptr<HamParty> meParty)
{
    ...
    this->nodeNames = nN;
    vector<string> nameParties = {"Alice", "Bob", "Charles", "Daniel", "Ezequiel", "Francis", "George", "Henry"};
    ...
    for(int i = 0; i< numOfParties; i++)
    {
        ...
        {
            nodeNames[index_n] = nameParties[i] + "-" + to_string(j);
            index_n += 1;
        }
    }
                ...
                for(int partyNum = 0; partyNum < numOfParties; partyNum++)
                {
                    ...
                    for(int k = 0; k < partyNumSeq; k++)
                    {
                        if( partyNum != i || j < k ) 
                        {
                            ...
                            string row_str = nameParties[i] + "-" + to_string(j);
                            int row = getIndex(nodeNames, row_str);

                            string column_str = nameParties[partyNum] + "-" + to_string(k);
