...
namespace etsi_qkd_004 {
    ...
    open_connect(const LKMSAddress &address, const URI &source, const URI &destination, const QoS &qos,
                 const UUID &key_stream_id = "",const std::string &role = "none") {
        // Request to json
        ...
        if (qos.metadata_mimetype != "") open_connect_json["data"]["qos"]["metadata_mimetype"] = qos.metadata_mimetype;
        if (key_stream_id != "") open_connect_json["data"]["key_stream_id"] = key_stream_id;
