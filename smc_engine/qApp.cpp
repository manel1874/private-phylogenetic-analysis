...
#include "include/qmatrixDist.hpp"
...
int main(int argc, char* argv[])
{
    ...
    cout << "Part 1.2 - GENERATE QKD AND QOKD KEYS" << endl;
    meHamParty->getAddressLKMS();
    meHamParty->getAddressQKD();
    meHamParty->getAddressQOKD();
    meHamParty->getKsidQKD();
    meHamParty->getKsidQOKD();
    meHamParty->generateQKDKeys();
    meHamParty->generateQOKDKeys();

    cout << "PART 1.3 - RUN HAMMING SMC \n" << endl;
    // Run Hamming SMC between every party present
    meHamParty->runHamSMC();

    cout << "PART 1.4 - SEND RESULTS \n" << endl;
    // Evaluators send (Garblers reveice) the results from Hamming SMC functionality
    meHamParty->evaluatorSendResultToGarbler();

    cout << "PART 1.5 - COMPUTE INTERNAL HAMMING DISTANCE \n" << endl;
    // Compute internal Hamming distances
    meHamParty->computeInternalHammingDistance();

    cout << "PART 1.6 - SEND/RECEIVE OTHER DISTANCES \n" << endl;
